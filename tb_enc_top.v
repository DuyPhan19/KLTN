...
//---- defines --------------------------------------------
  ...
  `define FRAME_WIDTH     1280 // (`FRAME_W/`LCU_SIZE + 1)*`LCU_SIZE  // full LCU
  `define FRAME_HEIGHT    720 // (`FRAME_H/`LCU_SIZE + 1)*`LCU_SIZE  // full LCU

  `define INITIAL_QP      20
  `define GOP_LENGTH      5
  `define FRAME_TOTAL     5
...
//---- test vectors ---------------------------------------
  `ifdef FORMAT_YUV
    `define FILE_CUR_YUV        "./tv/park_joy_420_720p50.yuv"
...
//--- main body ---------------------------------------------------
  ...
  initial begin
    fp = $fopen("bs_out.hevc", "wb");
    if (fp == 0) begin
	$display("Cannot open file!");
	$finish;
    end
    $fwrite(fp, "%c%c%c%c", 8'h00, 8'h00, 8'h00, 8'h01);    
  end

  always @(posedge clk) begin
    if (bs_val_o) begin 
	$fwrite(fp, "%c", bs_dat_o);
    end
  end
  final begin 
    $close(fp);
